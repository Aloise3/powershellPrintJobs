...
function Start-PrintJobMonitor {
    ...
    Register-WmiEvent -Query $query -SourceIdentifier $SourceIdentifier -Action { #Tjekker om der er et nyt event
        ...
        if ($ownerName = $userName) {
            ...
            try {
                #Udkommenteres da det er besværligt
                ...
                # Sender en mail
                #Send-MailMessage -From $senderEmail -To $recipientEmail -Subject "Print Job notifikation" -Body "Print job '$fileName' er blevet printet af '$ownerName'." -SmtpServer $SmtpServer #-Attachments $destinationPath
